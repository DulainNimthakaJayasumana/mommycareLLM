...
def contains_emoji(text: str) -> bool:
    """Check if the text contains any emojis using emoji.emoji_list()."""
    return len(emoji.emoji_list(text)) > 0


def extract_emojis(text: str) -> list:
    """Extract all emojis from the text using emoji.emoji_list()."""
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "Send your question in English or Sinhala. I'll reply with supportive, accurate information. You can also use emojis and I'll try to interpret what you mean."
    )
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle voice messages with an error response."""
    logger.info("Received voice message. Sending error response.")
    await update.message.reply_text(
        "Sorry, I encountered an error processing your voice message. Please send your question as text instead."
    )
